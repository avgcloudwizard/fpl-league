...
function maccapanti(){
 ...
 const captain=record.captains.find(p=>String(p.id)===String(hindsightPlayer))||record.captains[0];
 html+=`<section class="panel regret-section"><div class="panel-heading"><div><p class="eyebrow">I WAS GOING TO CAPTAIN HIM</p><h2>🎭 Captain Hindsight</h2></div></div><div class="regret-body"><p class="subtext">What if you had stuck with one player whenever he scored in your team?</p>${captain?`<label class="personal-filter">Your alternative captain<select id="hindsight-player">${record.captains.map(p=>`<option value="${p.id}" ${p.id===captain.id?'selected':''}>${esc(p.name)}</option>`).join('')}</select></label><div class="regret-summary"><div><small>Actual points</small><strong>${num(record.actual)}</strong></div><div><small>With ${esc(captain.name)}</small><strong>${num(record.actual+captain.delta)}</strong></div><div><small>Difference</small><strong>${signed(captain.delta)}</strong></div></div><p class="subtext">${esc(captain.name)} qualified in ${captain.weeks} GWs. ${captain.delta>0?'The armband had other ideas.':captain.delta<0?'Maybe your original decisions deserve some credit.':'Different armband. Same outcome.'}</p></div>${table(['GW','Player pts','Actual captain pts','Difference'],captain.scores.map(r=>row([cell('GW'+r.gw),cell(num(r.points)),cell(num(r.actual)),cell(signed(r.delta),'',r.delta)])))}<p class="table-note">Only weeks when this player earned points in your scoring lineup qualify, including autosubs and Bench Boost. Other weeks retain your actual captain. Captain points shown are before multipliers; the difference applies that GW’s normal or Triple Captain bonus. Actual points are completed-GW points after hits.</p>`:empty('Captain history unavailable.')+'</div>'}</section>`;
...
function untouched(){
 ...
 return heading('UnTouched','One squad. One armband. Absolutely no tinkering.')+regretNotice()+`<section class="panel"><div class="panel-heading"><h2>What if you left GW1 alone?</h2><span class="subtext">Through GW ${num(data.regrets?.through_gw)} · scroll for stats →</span></div>${table(['Position','Manager','Untouched pts','Actual pts','Untouched − actual','GW1 captain','GW1 vice'],ms.map((m,i)=>{const r=m.comparison,f=r?.frozen;if(f&&f.points!==previous){rank=i+1;previous=f.points;}return row([cell(f?medal(rank):'—'),cell(person(m)),cell(num(f?.points),'total'),cell(num(r?.actual)),cell(f?signed(f.points-r.actual):'—','',f?f.points-r.actual:''),cell(f?esc(f.captain):'—'),cell(f?esc(f.vice):'—')]);}),'untouched-table')}<p class="table-note">GW1 starting XI, bench order, captain and vice stay fixed. Automatic substitutions obey formation limits and bench priority. The vice takes over if the captain does not play. No chips, transfers, hits or manual changes—even if a chip was played in the actual GW1. Double GWs include all matches; blank-GW players do not play. Actual points use the same completed GWs, after hits. A positive difference means leaving it alone would have beaten the real team. Managers without a public GW1 squad are unavailable.</p></section>`;
}
function enemies(){
 const m=selectedManager(), squad=m?.public_team, gw=squad?.gw;
 let html=heading('Enemies','The players you are nervously watching from behind the sofa.',personalControl());
 if(!squad?.players?.length||squad.stale)return html+empty('Your latest public lineup is unavailable.');
 const rivals=data.managers.filter(p=>p.id!==m.id), ready=rivals.filter(p=>p.public_team?.gw===gw&&!p.public_team.stale&&p.public_team.players?.length===15);
 const own=new Map(squad.players.map(p=>[p.id,p.multiplier||0])), threats=new Map();
 for(const rival of ready)for(const p of rival.public_team.players){const exposure=(p.multiplier||0)-(own.get(p.id)||0);if(exposure<=0)continue;let t=threats.get(p.id);if(!t){t={...p,exposure:0,rivals:[]};threats.set(p.id,t);}t.exposure+=exposure;t.rivals.push({name:rival.name,team:rival.team,multiplier:p.multiplier,exposure});}
 const rows=[...threats.values()].sort((a,b)=>b.exposure-a.exposure||a.name.localeCompare(b.name));
 html+=`<section class="panel"><div class="panel-heading"><h2>Your GW ${num(gw)} watchlist</h2><span class="subtext">${ready.length}/${rivals.length} rivals available</span></div><p class="table-note">${squad.final?'Final GW lineups: this is a look back at who threatened your team.':'Latest public lineups: captain fallback and autosubs can still change.'} Each “swing” is how many points one player point gives those rivals collectively over you. It is exposure, not a predicted score or rank change.</p>${rows.length?table(['Player','Rivals gaining','Swing per point','Your multiplier'],rows.map(p=>row([cell(`<strong>${esc(p.name)}</strong><small class="subtext"> · ${esc(p.team)}</small>`),cell(p.rivals.map(r=>`<div class="enemy-rival">${esc(r.name)} <small>×${r.multiplier}</small></div>`).join('')),cell('+'+p.exposure,'total',p.exposure),cell('×'+(own.get(p.id)||0))]))):empty('No rival has more scoring exposure than you to any available player.')}<p class="table-note">Uses our 11 managers only. Bench players count only if their public multiplier scores points; captains, Triple Captain and Bench Boost are reflected. A player you own can still hurt if rivals captain him. Squads become public after the deadline. Delayed or mismatched squads are excluded.</p></section>`;
 return html;
}
...
load();
setInterval(()=>{if(!document.hidden&&!$('#profile').open&&!['creator-search','personal-manager','hindsight-player'].includes(document.activeElement?.id))load(true);},60000);
