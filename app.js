...
const liveNote = 'Live rank is the latest public overall FPL rank at the snapshot time; this site refreshes every 6 hours, not during every match event.';
...
function projectionTable(compact=false) {
 ...
 cell(`<div class="projection-bar"><span>${num(m.projected_total)} <small class="muted">(${m.projection_variance>0?'+':''}${num(m.projection_variance,2)}%)</small></span><div class="power-track"><i style="width:${Math.max(0,m.projected_total)/max*100}%"></i></div></div>`),cell(movement(m.rank-m.projected_position),'',m.rank-m.projected_position)])));
}
function home() {
 const ms=sortedManagers(), last=data.gameweeks.at(-1), winner=last?best(last.rows,'net'):[], high=best(ms,'best'), up=best(ms.filter(m=>m.movement>0),'movement'), down=best(ms.filter(m=>m.movement<0),'movement',true), power=best(ms,'power')[0];
 ...
 ${award('⚡ Highest Gameweek score',high,num(high[0]?.best)+' pts','Season record · after hits',{gws:'best_gws'})}
 ${award('📈 Biggest green arrow',up,up.length?'↑ '+up[0].movement+' '+(up[0].movement===1?'place':'places'):'','',{color:'up'})}
 ${award('📉 Biggest red arrow',down,down.length?'↓ '+Math.abs(down[0].movement)+' '+(down[0].movement===-1?'place':'places'):'','',{color:'down'})}
 ${award('🪑 Most bench points',best(ms,'bench'),num(best(ms,'bench')[0]?.bench)+' pts','Completed Gameweeks')}
 </div></section><section class="section panel"><div class="panel-heading"><h2>Projected final standings</h2><a class="text-link" href="#predictions">The full picture ↗</a></div>${projectionTable(true)}<p class="table-note">Simple form-based projection with a random 2–5% lift or dip in future points — not a guarantee.</p></section>`;
}
function gameweeks() {
...
function predictions(){return heading('The run-in, projected','A little form. A little arithmetic. Plenty still to play for.')+`<section class="panel formula"><div><p class="eyebrow">EXPECTED POINTS PER GAMEWEEK</p><div class="formula-math">65% recent form + 35% season form</div></div><p><strong>${data.total_gameweeks-data.completed_count} Gameweeks remaining.</strong> Recent form uses the last five available completed GWs. Expected future scoring gets a random 2–5% lift or dip for each manager, fixed until the next completed GW. Already-earned points stay unchanged.</p></section><section class="section panel"><div class="panel-heading"><h2>Projected final table</h2><span class="subtext">${data.total_gameweeks} Gameweeks</span></div>${projectionTable()}<p class="table-note">Simple form-based projection with a random 2–5% lift or dip in future points — not a guarantee.</p></section><details class="section panel" open><summary>Show the working</summary><p>Expected future GW score = (last 5 average × 0.65) + (season average × 0.35). Projected total = completed-GW total + (expected future score × remaining GWs × (1 + random swing)). Each manager gets a random positive or negative swing of 2–5%, seeded by manager and completed GW so the table stays consistent between visits. The swing disappears when no GWs remain. Scores include transfer hits. In-progress Gameweeks are excluded from averages and counted among the remaining GWs. Current points and current rank remain the official standings. Equal rounded projections share a finishing position. Injuries, fixtures and future chips are not modelled.</p></details>`;}
...
function prices() {
 ...
 `<section class="panel price-intro"><div><p class="eyebrow">${expired?'SAVED FORECAST DEADLINE':'NEXT PRICE UPDATE'} · YOUR LOCAL TIME</p><h2>${esc(deadline)}</h2></div><p>Forecasts are a guide, not a guarantee. They are progress towards a price-change threshold—not percentage probabilities. <a href="https://www.premierleague.com/en/news/4680462" target="_blank" rel="noopener noreferrer">About FPL's forecasts ↗</a></p></section>
 ...
 <p class="source-note">FPL forecast timestamp: ${watch.source_updated_at?esc(new Date(watch.source_updated_at).toLocaleString()):'Unavailable'}. Our saved snapshot updates every ${data.refresh_hours} hours. Check its timestamp before acting; these are not minute-by-minute updates.</p>`;
}

function creators() {
 const snapshot=data.creators, ms=snapshot?.managers||[];
 if(!ms.length)return heading('Content Creators','The familiar faces, in one table.')+empty('Creator standings are unavailable until the next successful refresh.');
 return heading('Content Creators',`${ms.length} creators. The same game. A different race.`)+(Date.now()-new Date(snapshot.updated_at).getTime()>18*3600000?'<div class="notice">The creator refresh is delayed. Showing the last complete creator snapshot.</div>':'')+
 `<section class="panel"><div class="panel-heading"><h2>Creator standings</h2><span class="subtext">Click any column to sort ↕</span></div>`+
 table(['Pos','Manager / team','Total','Last GW','↕','FTs*','Live rank'],ms.map(m=>row([cell(medal(m.rank),'rank-cell'),cell(person(m,false,true)),cell(num(m.total),'numeric total'),cell(num(m.latest_score),'numeric'),cell(movement(m.movement),'',m.movement??''),cell(ft(m)),cell(num(m.live_rank),'numeric')])),'standings creators-table')+
 `<p class="table-note">Positions and arrows are within this creator roster. Points ties use fewer non-chip transfers. Last GW is GW${snapshot.latest_completed??data.latest_completed}, the latest completed week in this creator snapshot. ${ftNote} ${liveNote}</p></section><p class="source-note">Roster from <a href="${esc(snapshot.source)}" target="_blank" rel="noopener noreferrer">FPLGameweek's Content Creators list ↗</a>, checked ${esc(snapshot.roster_checked_at)}. The viewer's comparison team is excluded. Scores refresh from official FPL data. Creator snapshot: ${esc(new Date(snapshot.updated_at).toLocaleString())}.</p>`;
}
function showCreatorProfile(m) {
 $('#profile-content').innerHTML=`<p class="eyebrow">CONTENT CREATOR</p><h2 id="profile-title">${esc(m.name)}</h2><p class="muted">${esc(m.team)}</p>${chipBadges(m)}<div class="profile-stats">${[['Creator position','#'+m.rank],['Total points',num(m.total)],['Last GW',num(m.latest_score)],['Overall FPL rank',num(m.live_rank)],['FTs remaining*',ft(m)]].map(([k,v])=>`<div><small>${k}</small><strong>${v}</strong></div>`).join('')}</div><p class="profile-note">${ftNote}</p><a class="text-link" href="https://fantasy.premierleague.com/entry/${m.id}/history" target="_blank" rel="noopener noreferrer">View public FPL team ↗</a>`;
 $('#profile').showModal();
}

function showProfile(id){const m=[...data.managers,...(data.creators?.managers||[])].find(m=>m.id===Number(id));if(!m)return;if(m.creator){showCreatorProfile(m);return;}const max=Math.max(1,...m.recent.map(r=>r.net));
...
function render(){if(!data)return;const route=location.hash.slice(1)||'home';const pages={home,season,gameweeks,monthly,predictions,prices,creators};const active=pages[route]?route:'home';document.querySelectorAll('[data-nav]').forEach(a=>{a.classList.toggle('active',a.dataset.nav===active);if(a.dataset.nav===active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});
 ...
 document.title=`${active==='home'?data.name:active==='creators'?'Content Creators':active[0].toUpperCase()+active.slice(1)} · Touchline`;
}
...
document.addEventListener('click',event=>{const sort=event.target.closest('[data-sort]');if(sort){sortTable(sort);return;}const button=event.target.closest('[data-manager]');if(button)showProfile(button.dataset.manager);});
