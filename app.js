...
function home() {
 ...
 `<div class="layout"><section class="panel"><div class="panel-heading"><h2>League standings</h2><span class="subtext">${data.in_progress?'Provisional':'Overall'}</span></div>${ms.length?table(['Pos','Manager / team','Total','Last GW','↕','FTs*','Live rank'],ms.map(m=>row([cell(medal(m.rank),'rank-cell'),cell(person(m,true,true),'standing-manager'),cell(num(m.total),'numeric total'),cell(num(m.latest_score),'numeric'),cell(movement(m.movement),'movement-cell',m.movement??''),cell(ft(m)),cell(num(m.live_rank),'numeric')],m.rank===1?'leader-row':m.rank===7?'seventh-row':m.rank===ms.at(-1).rank?'last-row':'')),'standings rank-portraits'):empty('No managers have scored yet.')}<p class="table-note">Last GW = points after hits. Swipe the table for FTs and live ranks. Select a manager for their stats.</p><p class="table-note">${ftNote} ${liveNote}</p></section>
