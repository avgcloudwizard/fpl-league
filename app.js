...
function untouched(){
 ...
 return heading('UnTouched','One squad. One armband. Absolutely no tinkering.')+regretNotice()+`<section class="panel"><div class="panel-heading"><h2>What if you left GW1 alone?</h2><span class="subtext">Through GW ${num(data.regrets?.through_gw)} · scroll for stats →</span></div>${table(['Frozen table pos','Manager','Untouched rank','Untouched pts','Actual pts','Untouched − actual','GW1 captain','GW1 vice'],ms.map((m,i)=>{const r=m.comparison,f=r?.frozen;if(f&&f.points!==previous){rank=i+1;previous=f.points;}return row([cell(f?medal(rank):'—'),cell(person(m)),cell(f?num(1+records.filter(other=>other.id!==m.id&&other.actual>f.points).length):'—'),cell(num(f?.points),'total'),cell(num(r?.actual)),cell(f?signed(f.points-r.actual):'—','',f?f.points-r.actual:''),cell(f?esc(f.captain):'—'),cell(f?esc(f.vice):'—')]);}),'untouched-table')}<p class="table-note">Untouched rank compares your frozen-team points with everyone else’s actual points; equal points share a rank. Frozen table position compares all frozen teams. GW1 starting XI, bench order, captain and vice stay fixed. Automatic substitutions obey formation limits and bench priority. The vice takes over if the captain does not play. No chips, transfers, hits or manual changes—even if a chip was played in the actual GW1. Double GWs include all matches; blank-GW players do not play. Actual points use the same completed GWs, after hits. A positive difference means leaving it alone would have beaten the real team. Managers without a public GW1 squad are unavailable.</p></section>`;
}
...
load();
setInterval(()=>{if(!document.hidden&&!$('#profile').open&&!['creator-search','enemy-search','personal-manager','hindsight-player'].includes(document.activeElement?.id))load(true);},60000);
