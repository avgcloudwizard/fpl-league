...
function home() {
 const ms=sortedManagers(), last=data.gameweeks.at(-1), winner=last?best(last.rows,'net'):[], high=best(ms,'best'), up=best(ms.filter(m=>m.overall_movement_percent>0),'overall_movement_percent'), down=best(ms.filter(m=>m.overall_movement_percent<0),'overall_movement_percent',true), power=best(ms,'power')[0];
 ...
 ${award('📈 Biggest green arrow',up,up.length?'↑ '+num(up[0].overall_movement_percent,1)+'%':'','Overall rank improvement vs previous GW',{color:'up'})}
 ${award('📉 Biggest red arrow',down,down.length?'↓ '+num(Math.abs(down[0].overall_movement_percent),1)+'%':'','Overall rank decline vs previous GW',{color:'down'})}
 ${award('🪑 Most bench points',best(ms,'bench'),num(best(ms,'bench')[0]?.bench)+' pts','Completed Gameweeks')}
 </div></section><section class="section panel"><div class="panel-heading"><h2>Projected final standings</h2><a class="text-link" href="#predictions">The full picture ↗</a></div>${projectionTable(true)}<p class="table-note">Conservative projection using recent form, season form and chips remaining — not a guarantee.</p></section>`;
}
function gameweeks() {
...
function predictions(){return heading('The run-in, projected','Recent form. Chips in reserve. Plenty still to play for.')+`<section class="panel formula"><div><p class="eyebrow">FORM + CHIPS REMAINING</p><div class="formula-math">65% recent form + 35% season form</div></div><p><strong>${data.total_gameweeks-data.latest_completed} Gameweeks remaining.</strong> Recent returns are balanced with season form and a conservative scoring baseline. Unused chips add a small allowance within their first- or second-half windows.</p></section><section class="section panel"><div class="panel-heading"><h2>Projected final table</h2><span class="subtext">${data.total_gameweeks} Gameweeks</span></div>${projectionTable()}<p class="table-note">A simple statistical scenario, not a guarantee. Actual points are never reduced.</p></section><details class="section panel"><summary>Show the working</summary><p>The last five completed GW scores carry 65% of the form blend; season form carries 35%. Early-season returns are moderated towards a 50-point ordinary GW baseline, with greater weight on observed form as more weeks are completed. Unused Wildcard, Free Hit and Triple Captain add 8 estimated points each; Bench Boost adds 12. Expired chips are excluded and only one chip can be used in each remaining GW. A conservative season envelope dampens extreme extrapolations. In-progress weeks are not counted twice. These are modelling assumptions, not promises of chip returns; fixtures and injuries are not modelled.</p></details>`;}
...
function teamLink(m){return `https://fantasy.premierleague.com/entry/${m.id}/event/${m.public_team?.gw||data.current_gw||data.latest_completed}`;}
