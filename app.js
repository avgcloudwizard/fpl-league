...
function creators() {
 ...
 return heading('Content Creators',`${ms.length-ownCount} creators + ${ownCount} of us. See where we stand.`)+(Date.now()-new Date(snapshot.updated_at).getTime()>18*3600000?'<div class="notice">The creator refresh is delayed. Creator scores use the last complete creator snapshot; our league uses its latest snapshot.</div>':'')+
 `<section class="panel"><div class="panel-heading"><h2>Creator standings</h2><span class="subtext">Click any column to sort ↕</span></div>`+
 table(['Pos','Manager / team','Total','Last GW','↕','FTs*','Live rank'],ms.map(m=>row([cell(medal(m.rank),'rank-cell'),cell(person(m,false,true)+(m.league_member?'<span class="league-tag">Our league</span>':'')),cell(num(m.total),'numeric total'),cell(num(m.latest_score),'numeric'),cell(movement(m.movement),'',m.movement??''),cell(ft(m)),cell(num(m.live_rank),'numeric')],m.league_member?'league-member':'')),'standings creators-table')+
