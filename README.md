...
Each scheduled update fetches each creator's public entry and history from official FPL. The table has the same columns as Home, with creator name and actual team name, chip badges, rank among all teams in the combined table, net last completed GW score, rank movement within that combined table, calculated FTs and overall FPL rank. Ties use fewer non-chip transfers. Click a creator for their compact details. Our own manager pop-ups retain mini-league position and show overall FPL rank (OR rank) in place of Power Rating. The combined table never changes the ranks shown on Home or Season. Duplicate entry IDs appear only once.
